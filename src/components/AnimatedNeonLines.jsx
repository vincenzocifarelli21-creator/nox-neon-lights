...
const AnimatedNeonLines = ({ className = '', intensity = 'normal', safeArea = null }) => {
  ...
  // Define only horizontal lines (6), colors inspired by reference (blue and orange)
  const lineConfigs = [
    {
      type: 'horizontal',
  ...
  return (
    <div className={`fixed inset-0 pointer-events-none overflow-hidden ${className}`}>
      ...
      >
        ...
        <g mask={safeArea ? "url(#safe-area-mask)" : undefined}>
          {lineConfigs.map((line, index) => {
            ...
            return (
              <g key={index}>
                <motion.line
                  ...
                  stroke={line.color} strokeWidth="12" strokeOpacity="0.1"
                  filter={`url(#blur-${index})`}
                  initial={{ pathLength: 0, opacity: 0 }}
                  animate={{ pathLength: [0, 1, 0.8, 1], opacity: [0, 0.6, 0.3, 0.7] }}
                  transition={{ duration: line.duration, delay: line.delay, repeat: Infinity, repeatType: "loop", ease: "easeInOut" }}
                />
                <motion.line
                  x1={startX} y1={startY} x2={endX} y2={endY}
                  stroke={line.color} strokeWidth="6" strokeOpacity="0.9" strokeLinecap="round"
                  initial={{ pathLength: 0, opacity: 0 }}
                  animate={{ pathLength: [0, 1, 0.9, 1], opacity: [0, 1, 0.7, 0.95] }}
                  transition={{ duration: line.duration, delay: line.delay, repeat: Infinity, repeatType: "loop", ease: "easeInOut" }}
                  style={{ filter: `drop-shadow(0 0 8px ${line.color}) drop-shadow(0 0 16px ${line.color}40)` }}
                />
                <motion.line
                  x1={startX} y1={startY} x2={endX} y2={endY}
                  stroke="white" strokeWidth="2" strokeOpacity="0.8" strokeLinecap="round"
                  initial={{ pathLength: 0, opacity: 0 }}
                  animate={{ pathLength: [0, 1, 0.95, 1], opacity: [0, 0.9, 0.6, 0.8] }}
                  transition={{ duration: line.duration, delay: line.delay + 0.1, repeat: Infinity, repeatType: "loop", ease: "easeInOut" }}
                />
                <motion.line
                  x1={startX + lengthPx * 0.3} y1={startY} x2={startX + lengthPx * 0.7} y2={startY}
                  stroke="white" strokeWidth="1" strokeOpacity="0.9" strokeLinecap="round"
                  initial={{ opacity: 0 }}
                  animate={{ opacity: [0, 1, 0, 0.8, 0, 1, 0] }}
                  transition={{ duration: line.duration * 0.5, delay: line.delay + 1, repeat: Infinity, repeatType: "loop", ease: "easeInOut" }}
                />
              </g>
            );
          })}
